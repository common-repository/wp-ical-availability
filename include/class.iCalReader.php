...
class ICal
{
     ...
     */
    public function __construct($wpia_host=false)
    {
        if (!$wpia_host) {
            return false;
        }
        ...
        $wpia_host  = urldecode( $wpia_host );

        // $wpia_host  = $wpia_host['scheme'] . '://' . $wpia_host['host'] . $wpia_host['path'] . urldecode($wpia_host['query']) . urldecode( $wpia_host['fragment'] );
        $wpia_host  = str_replace(array("038;","#"),"",$wpia_host);

        
        curl_setopt($wpia_ch,    CURLOPT_USERAGENT,          $agent);
		curl_setopt($wpia_ch,    CURLOPT_URL,                $wpia_host );
        curl_setopt($wpia_ch,    CURLOPT_RETURNTRANSFER,     true);
        curl_setopt($wpia_ch,    CURLOPT_FOLLOWLOCATION,     true);
        curl_setopt($wpia_ch,    CURLOPT_HEADER,             false);
        curl_setopt($wpia_ch,    CURLOPT_SSL_VERIFYPEER,     false);
        curl_setopt($wpia_ch,    CURLOPT_CONNECTTIMEOUT,     7);
        curl_setopt($wpia_ch,    CURLOPT_TIMEOUT,            5);
        
        $contents = curl_exec($wpia_ch);
        // echo '<pre>';var_dump(curl_error($wpia_ch));echo '</pre>';
        // echo'<pre>';var_dump(curl_getinfo($wpia_ch));echo'</pre>';
